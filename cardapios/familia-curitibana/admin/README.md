# Painel Administrativo — Família Curitibana
...
4. Em **Authentication → Users**, clique em **Add user** e crie o login do dono
   (ex.: `dona@familia-curitibana.com`)
...
2. Copie os valores e preencha em `firebase-config.js`:
   ...
   ```
3. O `RESTAURANT_ID = "familia-curitibana"` já está configurado. **Não altere**.
...
No documento `restaurants/familia-curitibana/settings/info`, crie/adicione:
```json
{
...
├── css/
│   ├── style.css        (estilo do painel, paleta verde + dourado)
...
```
restaurants/familia-curitibana/
  settings/
